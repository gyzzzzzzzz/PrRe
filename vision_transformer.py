...
from prompt import FSCILPrompt
...
class Attention(nn.Module): 
    ...
    def forward(self, x, *args):
        ...
        # q.shape=torch.Size([batch_size, 12, 197, 64])
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            f_prompt_length=None, embedding_key='cls', prompt_init='uniform', prompt_pool=False, prompt_key=False, pool_size=None,
            top_k=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', usf_prompt_mask=False,
            use_d_prompt=False, d_prompt_length=None, d_prompt_layer_idx=None, use_prefix_tune_for_d_prompt=False,
            use_f_prompt=False, f_prompt_layer_idx=None, use_prefix_tune_for_f_prompt=False, same_key_value=False,):
        """
        Args:
        ...
        self.usf_prompt_mask = usf_prompt_mask 

        self.use_d_prompt = use_d_prompt 
        self.d_prompt_layer_idx = d_prompt_layer_idx 
        # num_d_prompt : The actual number of layers to which g-prompt is attached.
        # In official code, create as many layers as the total number of layers and select them based on the index
        num_d_prompt = len(self.d_prompt_layer_idx) if self.d_prompt_layer_idx is not None else 0 
        self.use_prefix_tune_for_d_prompt = use_prefix_tune_for_d_prompt 
        
        self.use_f_prompt = use_f_prompt 
        if self.use_f_prompt:
            self.f_prompt_layer_idx = f_prompt_layer_idx  
        else:
            self.f_prompt_layer_idx = [-1]
        num_f_prompt = len(self.f_prompt_layer_idx) if self.f_prompt_layer_idx is not None else 0 
        self.use_prefix_tune_for_f_prompt = use_prefix_tune_for_f_prompt 
        
        if not self.use_prefix_tune_for_d_prompt and not self.use_prefix_tune_for_d_prompt: 
            self.use_d_prompt = False
            self.d_prompt_layer_idx = []
#d_prompt
        if use_d_prompt and d_prompt_length is not None and len(d_prompt_layer_idx) != 0: 
            if not use_prefix_tune_for_d_prompt: 
                d_prompt_shape=(num_d_prompt, d_prompt_length, embed_dim)
                if prompt_init == 'zero':
                    self.d_prompt = nn.Parameter(torch.zeros(d_prompt_shape))
                elif prompt_init == 'uniform':
                    self.d_prompt = nn.Parameter(torch.randn(d_prompt_shape))
                    nn.init.uniform_(self.d_prompt, -1, 1)
            else:
                if same_key_value: 
                    d_prompt_shape=(num_d_prompt, 1, d_prompt_length, num_heads, embed_dim // num_heads)
                    if prompt_init == 'zero':
                        self.d_prompt = nn.Parameter(torch.zeros(d_prompt_shape))
                    elif prompt_init == 'uniform':
                        self.d_prompt = nn.Parameter(torch.randn(d_prompt_shape))
                        nn.init.uniform_(self.d_prompt, -1, 1)
                    self.d_prompt = self.d_prompt.repeat(1, 2, 1, 1, 1)
                else:
                    d_prompt_shape=(num_d_prompt, 2, d_prompt_length, num_heads, embed_dim // num_heads) 
                    if prompt_init == 'zero':
                        self.d_prompt = nn.Parameter(torch.zeros(d_prompt_shape))
                    elif prompt_init == 'uniform': 
                        self.d_prompt = nn.Parameter(torch.randn(d_prompt_shape))
            ...
            self.d_prompt = None 
#f_prompt        
        if use_f_prompt and f_prompt_layer_idx is not None: 
            self.f_prompt = FSCILPrompt(length=f_prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
                    prompt_pool=prompt_pool, prompt_key=prompt_key, pool_size=pool_size, top_k=top_k, batchwise_prompt=batchwise_prompt,
                    prompt_key_init=prompt_key_init, num_layers=num_f_prompt, use_prefix_tune_for_f_prompt=use_prefix_tune_for_f_prompt,
                    num_heads=num_heads, same_key_value=same_key_value)
        
        if not (use_d_prompt or use_f_prompt): 
            attn_layer = Attention 
        elif not (use_prefix_tune_for_d_prompt or use_prefix_tune_for_f_prompt):
            # Prompt tunning
            attn_layer = Attention
        else:
            # Prefix tunning
        ...
        if self.prompt_pool: 
            if not self.use_prefix_tune_for_d_prompt:
                self.total_prompt_len += d_prompt_length * len(self.d_prompt_layer_idx)
            if not self.use_prefix_tune_for_f_prompt:
                self.total_prompt_len += f_prompt_length * top_k * len(self.f_prompt_layer_idx)
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting(): 
            ...
            if self.use_d_prompt or self.use_f_prompt: 
                if self.usf_prompt_mask and train and self.use_f_prompt: 
                    start = task_id * self.f_prompt.top_k 
                    end = (task_id + 1) * self.f_prompt.top_k 
                    singlf_prompt_mask = torch.arange(start, end).to(x.device) 
                    prompt_mask = singlf_prompt_mask.unsqueeze(0).expand(x.shape[0], -1) 
                    if end > self.f_prompt.pool_size:
                        prompt_mask = None
                else:
                    prompt_mask = None 
                ...
                if self.use_f_prompt:
                    res = self.f_prompt(x, prompt_mask=prompt_mask, cls_features=cls_features)
                     
                    f_prompt = res['batched_prompt'] 
                else:
                    res = dict()

                for i, block in enumerate(self.blocks):
                    if i in self.d_prompt_layer_idx: 
                        if self.use_prefix_tune_for_d_prompt and self.use_d_prompt:
                            d_prompt_counter += 1
                            # Prefix tunning, [B, 2, d_prompt_length, num_heads, embed_dim // num_heads]
                            idx = torch.tensor([d_prompt_counter] * x.shape[0]).to(x.device) 
                            d_prompt = self.d_prompt[idx] 
                        else:                                                  
                            d_prompt=None
                        x = block(x, prompt=d_prompt) 
                    
                    elif i in self.f_prompt_layer_idx:
                        f_prompt_counter += 1
                        if self.use_prefix_tune_for_f_prompt:
                           
                            x = block(x, prompt=f_prompt[f_prompt_counter]) 
                        else:
                            # Pommpt tunning, [B, top_k * f_prompt_length, embed_dim]
                            prompt = f_prompt[f_prompt_counter]
        ...
        return res,d_prompt,f_prompt,pos_embed
    ...
    def forward(self, x, task_id=-1, cls_features=None, train=False):
        res,d_prompt,f_prompt,pos_embed = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train)
