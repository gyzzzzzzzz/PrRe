...
#封装了对CIFAR-10数据集的处理和加载功能，提供了下载、验证、转换和读取数据的方法，方便了对数据集的处理和使用
class CIFAR10(VisionDataset):
    ...
    tgz_md5 = 'c58f30108f718f92721af3b95e74349a' # 文件的MD5校验和，用于验证下载文件的完整性
    train_list = [
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False, index=None, base_sess=None,
                 two_images=False, validation=False):
        ...
        if self.train:
            ...
            if transform:
                ...
                if validation:
                    ...
                    if two_images:
                        print('---- CIFAR100 TwoCrops Training Transform ---')
            ...
            if transform:
                ...
                if validation:
                    ...
                    print('---- CIFAR100 Testing Transform ---')
        ...
        #遍历downloaded_list列表中的每个文件，打开每个文件并从中加载数据，
        #然后将加载的数据添加到self.data列表中，同时将标签数据添加到self.targets列表中。
    ...
    #获取数据集中指定索引的数据和对应的标签，并进行相应的数据转换和处理
    ...
    def _check_integrity(self):
        #这段代码的作用是遍历数据集的训练集和测试集文件列表，
        #检查每个文件的完整性。如果发现任何一个文件校验失败，立即返回False；
        #只有所有文件都校验成功，才会最终返回True，表示数据集完整。
        root = self.root
...
class TwoCropsTransform:
    ...
    def __call__(self, x):#同一图像生成两个增强图像，增加数据集多样性
